  ...
  class EndpointDeployApiGateway extends S.classes.Plugin {
    ...
    _createDeployment(region) {
      ...
      return new BbPromise( function( resolve, reject ){
        ...
        // allow overrides for functionAlias stage-region variable and  and -i, --aliasRestApi flag
        let functionAlias;
        if (_this.evt.options.aliasRestApi) functionAlias  = _this.evt.options.aliasRestApi;
        else functionAlias = _this.project.getRegion(_this.evt.options.stage, _this.evt.options.region).getVariables().functionAlias;
          ...
          let params = {
            ...
            variables: {
              functionAlias:  functionAlias || _this.evt.options.stage
            }
          };
